...
export default function PayinPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await api.post('/api/aggregator/payin/', payload);
  ...
  return (
    <div style={{ maxWidth: 600, margin: '0 auto' }}>
      <div style={{ marginBottom: 'var(--space-xl)' }}>
        <h1 style={{ fontSize: '1.5rem', fontWeight: 800 }}>{t('payin.title')}</h1>
        <p style={{ color: 'var(--text-tertiary)', fontSize: '0.875rem', marginTop: 4 }}>{t('payin.subtitle')}</p>
      </div>
      ...
      <div className="card">
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-xl)' }}>
          ...
          <FormField label={t('payin.objectLabel')}>
            <input type="text" name="object" placeholder={t('payin.objectPlaceholder')} required style={inputStyle} />
          </FormField>
          ...
          >
            {loading ? t('common.processing') : t('payin.submitButton')}
          </button>
        </form>
      </div>
    </div>
